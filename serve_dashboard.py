...
class DashboardHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/api/guided-answer":
    ...
    def do_POST(self):
        ...
        try:
            ...
            print(f"[guided-answer] provider={provider} model={model} question={payload.get('question')!r}")
            print(f"[guided-answer] answer-length={len(answer)} answer-preview={answer[:300]!r}")
            self.respond_json({"ok": True, "answer": answer, "provider": provider, "model": model})
        except RuntimeError as exc:
            print(f"[guided-answer] runtime-error: {exc}")
            self.respond_json({"ok": False, "error": str(exc)}, status=HTTPStatus.SERVICE_UNAVAILABLE)
        except HTTPError as exc:
            detail = exc.read().decode("utf-8", errors="ignore")
            print(f"[guided-answer] upstream-http-error code={exc.code} detail={detail}")
            self.respond_json(
                {"ok": False, "error": f"Upstream API error {exc.code}", "detail": detail},
                status=HTTPStatus.BAD_GATEWAY,
            )
        except URLError as exc:
            print(f"[guided-answer] network-error: {exc.reason}")
            self.respond_json(
                {"ok": False, "error": f"Network error: {exc.reason}"},
                status=HTTPStatus.BAD_GATEWAY,
            )
        except Exception as exc:
            print(f"[guided-answer] unexpected-error: {exc}")
            self.respond_json(
...
def main():
    ...
    provider_status = get_provider_status()
    print(f"Serving dashboard at http://{HOST}:{PORT}/dashboard/index_llm_recommend.html")
    print(
        "Detected keys:",
        {
            "LOCAL_ENV_FILE": bool((ROOT / ".env.local").exists()),
            "resolved_provider": provider_status["resolved_provider"],
            "ready": provider_status["ready"],
            "active_model": provider_status["active_model"],
            "OPENAI_API_KEY": provider_status["has_openai_key"],
            "GEMINI_API_KEY": provider_status["has_gemini_key"],
            "KAKAO_API_KEY": provider_status["has_kakao_key"],
            "LLM_API_KEY": provider_status["has_generic_key"],
        },
    )
    print("Set LLM_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, or KAKAO_API_KEY to enable guided answers.")
